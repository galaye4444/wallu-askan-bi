...
if(isset($_POST[submit]))
{
	if(isset($_GET[editid]))
	{
		...
		{
			echo "<script>alert('department mise a jour avec success...');</script>";
		}
		else
		{
	...
	if($qsql = mysqli_query($con,$sql))
	{
		echo "<script>alert('Department ajouter avec success...');</script>";
	}
	else
	{
		echo mysqli_error($con);
...
<div class="container-fluid">
	<div class="block-header">
            <h2>Ajouter Nouveau Department </h2>
  ...
  <div class="card">
    ...
    <table class="table table-hover">
      <tbody>
        <tr>
          <td width="34%">Nom Department</td>
          <td width="66%"><input placeholder=" Ecrire ici " class="form-control" type="text" name="departmentname" id="departmentname" value="<?php echo $rsedit[departmentname]; ?>" /></td>
        </tr>
        <tr>
          <td>Description</td>
          <td><textarea placeholder=" Ecrire ici  " class="form-control no-resize" name="textarea" id="textarea" cols="45" rows="5"><?php echo $rsedit[description] ; ?></textarea></td>
        </tr>
        <tr>
          <td>Status</td>
